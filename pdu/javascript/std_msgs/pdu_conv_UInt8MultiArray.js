...
/**
 * Deserializes a binary PDU into a UInt8MultiArray object.
 * @param {ArrayBuffer} binary_data
 * @returns { UInt8MultiArray }
 */
export function pduToJs_UInt8MultiArray(binary_data) {
    const js_obj = new UInt8MultiArray();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_UInt8MultiArray(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_UInt8MultiArray(meta, binary_data, js_obj, base_off) {
...
/**
 * Serializes a UInt8MultiArray object into a binary PDU.
 * @param { UInt8MultiArray } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_UInt8MultiArray(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_UInt8MultiArray(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_UInt8MultiArray(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: layout, type: MultiArrayLayout (struct)
