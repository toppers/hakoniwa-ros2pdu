...
/**
 * Deserializes a binary PDU into a Int16MultiArray object.
 * @param {ArrayBuffer} binary_data
 * @returns { Int16MultiArray }
 */
export function pduToJs_Int16MultiArray(binary_data) {
    const js_obj = new Int16MultiArray();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_Int16MultiArray(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_Int16MultiArray(meta, binary_data, js_obj, base_off) {
    ...
    // member: data, type: int16 (primitive)
    ...
    { // varray
        ...
        if ("int16" === 'string') {
            ...
            js_obj.data = PduUtils.binToArrayValues("int16", array_bin, array_size);
        }
    }
...
/**
 * Serializes a Int16MultiArray object into a binary PDU.
 * @param { Int16MultiArray } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_Int16MultiArray(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_Int16MultiArray(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_Int16MultiArray(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: layout, type: MultiArrayLayout (struct)
    ...
    { // varray
        ...
        if ("int16" === 'string') {
            ...
            data_buffer = PduUtils.typesToBin("int16", js_obj.data);
