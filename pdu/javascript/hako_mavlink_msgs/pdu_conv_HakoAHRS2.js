import * as PduUtils from '../pdu_utils.js';
import { HakoAHRS2 } from './pdu_jstype_HakoAHRS2.js';


/**
 * Deserializes a binary PDU into a HakoAHRS2 object.
 * @param {ArrayBuffer} binary_data
 * @returns { HakoAHRS2 }
 */
export function pduToJs_HakoAHRS2(binary_data) {
    const js_obj = new HakoAHRS2();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_HakoAHRS2(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_HakoAHRS2(meta, binary_data, js_obj, base_off) {
    const view = new DataView(binary_data);
    const littleEndian = true;
    // member: roll, type: float32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 0, 4);
        js_obj.roll = PduUtils.binToValue("float32", bin);
    }
    
    // member: pitch, type: float32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 4, 4);
        js_obj.pitch = PduUtils.binToValue("float32", bin);
    }
    
    // member: yaw, type: float32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 8, 4);
        js_obj.yaw = PduUtils.binToValue("float32", bin);
    }
    
    // member: altitude, type: float32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 12, 4);
        js_obj.altitude = PduUtils.binToValue("float32", bin);
    }
    
    // member: lat, type: int32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 16, 4);
        js_obj.lat = PduUtils.binToValue("int32", bin);
    }
    
    // member: lng, type: int32 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 20, 4);
        js_obj.lng = PduUtils.binToValue("int32", bin);
    }
    
    return js_obj;
}

/**
 * Serializes a HakoAHRS2 object into a binary PDU.
 * @param { HakoAHRS2 } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_HakoAHRS2(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_HakoAHRS2(0, bw_container, base_allocator, js_obj);

    const base_data_size = base_allocator.size();
    const heap_data_size = bw_container.heap_allocator.size();
    
    bw_container.meta.heap_off = PduUtils.PDU_META_DATA_SIZE + base_data_size;
    bw_container.meta.total_size = bw_container.meta.heap_off + heap_data_size;

    const final_buffer = new ArrayBuffer(bw_container.meta.total_size);
    const final_view = new Uint8Array(final_buffer);

    PduUtils.writeBinary(final_view, 0, bw_container.meta.to_bytes());
    PduUtils.writeBinary(final_view, bw_container.meta.base_off, base_allocator.toArray());
    PduUtils.writeBinary(final_view, bw_container.meta.heap_off, bw_container.heap_allocator.toArray());

    return final_buffer;
}

export function binary_write_recursive_HakoAHRS2(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: roll, type: float32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("float32", js_obj.roll, 4);
        allocator.add(bin, parent_off + 0);
    }
    
    // member: pitch, type: float32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("float32", js_obj.pitch, 4);
        allocator.add(bin, parent_off + 4);
    }
    
    // member: yaw, type: float32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("float32", js_obj.yaw, 4);
        allocator.add(bin, parent_off + 8);
    }
    
    // member: altitude, type: float32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("float32", js_obj.altitude, 4);
        allocator.add(bin, parent_off + 12);
    }
    
    // member: lat, type: int32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("int32", js_obj.lat, 4);
        allocator.add(bin, parent_off + 16);
    }
    
    // member: lng, type: int32 (primitive)

    
    {
        const bin = PduUtils.typeToBin("int32", js_obj.lng, 4);
        allocator.add(bin, parent_off + 20);
    }
    
}
