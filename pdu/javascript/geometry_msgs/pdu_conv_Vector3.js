...
/**
 * Deserializes a binary PDU into a Vector3 object.
 * @param {ArrayBuffer} binary_data
 * @returns { Vector3 }
 */
export function pduToJs_Vector3(binary_data) {
    const js_obj = new Vector3();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_Vector3(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_Vector3(meta, binary_data, js_obj, base_off) {
...
/**
 * Serializes a Vector3 object into a binary PDU.
 * @param { Vector3 } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_Vector3(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_Vector3(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_Vector3(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: x, type: float64 (primitive)
