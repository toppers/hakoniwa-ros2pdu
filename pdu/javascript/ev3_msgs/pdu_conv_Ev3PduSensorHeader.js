...
/**
 * Deserializes a binary PDU into a Ev3PduSensorHeader object.
 * @param {ArrayBuffer} binary_data
 * @returns { Ev3PduSensorHeader }
 */
export function pduToJs_Ev3PduSensorHeader(binary_data) {
    const js_obj = new Ev3PduSensorHeader();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_Ev3PduSensorHeader(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_Ev3PduSensorHeader(meta, binary_data, js_obj, base_off) {
    ...
    // member: hakoniwa_time, type: int64 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 136, 8);
        js_obj.hakoniwa_time = PduUtils.binToValue("int64", bin);
    }
...
/**
 * Serializes a Ev3PduSensorHeader object into a binary PDU.
 * @param { Ev3PduSensorHeader } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_Ev3PduSensorHeader(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_Ev3PduSensorHeader(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_Ev3PduSensorHeader(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: name, type: string (primitive)
    ...
    {
        const bin = PduUtils.typeToBin("int64", js_obj.hakoniwa_time, 8);
