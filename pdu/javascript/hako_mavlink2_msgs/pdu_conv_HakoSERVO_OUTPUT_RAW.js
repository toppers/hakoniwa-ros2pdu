...
export function binary_read_recursive_HakoSERVO_OUTPUT_RAW(meta, binary_data, js_obj, base_off) {
    ...
    // member: servo9_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 22, 2);
        js_obj.servo9_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo10_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 24, 2);
        js_obj.servo10_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo11_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 26, 2);
        js_obj.servo11_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo12_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 28, 2);
        js_obj.servo12_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo13_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 30, 2);
        js_obj.servo13_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo14_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 32, 2);
        js_obj.servo14_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo15_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 34, 2);
        js_obj.servo15_raw = PduUtils.binToValue("uint16", bin);
    }
    
    // member: servo16_raw, type: uint16 (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 36, 2);
        js_obj.servo16_raw = PduUtils.binToValue("uint16", bin);
    }
...
export function binary_write_recursive_HakoSERVO_OUTPUT_RAW(parent_off, bw_container, allocator, js_obj) {
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo9_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo10_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo11_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo12_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo13_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo14_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo15_raw, 2);
    ...
    {
        const bin = PduUtils.typeToBin("uint16", js_obj.servo16_raw, 2);
