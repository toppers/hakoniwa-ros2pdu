...
/**
 * Deserializes a binary PDU into a CameraCaptureImageResponse object.
 * @param {ArrayBuffer} binary_data
 * @returns { CameraCaptureImageResponse }
 */
export function pduToJs_CameraCaptureImageResponse(binary_data) {
    const js_obj = new CameraCaptureImageResponse();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_CameraCaptureImageResponse(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_CameraCaptureImageResponse(meta, binary_data, js_obj, base_off) {
    ...
    { // varray
        const array_size = view.getInt32(base_off + 4, littleEndian);
        const offset_from_heap = view.getInt32(base_off + 4 + 4, littleEndian);
...
/**
 * Serializes a CameraCaptureImageResponse object into a binary PDU.
 * @param { CameraCaptureImageResponse } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_CameraCaptureImageResponse(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_CameraCaptureImageResponse(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_CameraCaptureImageResponse(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: ok, type: bool (primitive)
    ...
    { // varray
        ...
        allocator.add(ref_buffer, parent_off + 4);
    }
