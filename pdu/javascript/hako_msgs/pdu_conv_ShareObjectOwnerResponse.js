...
/**
 * Deserializes a binary PDU into a ShareObjectOwnerResponse object.
 * @param {ArrayBuffer} binary_data
 * @returns { ShareObjectOwnerResponse }
 */
export function pduToJs_ShareObjectOwnerResponse(binary_data) {
    const js_obj = new ShareObjectOwnerResponse();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_ShareObjectOwnerResponse(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_ShareObjectOwnerResponse(meta, binary_data, js_obj, base_off) {
    ...
    // member: accepted, type: bool (primitive)

    
    {
        const bin = PduUtils.readBinary(binary_data, base_off + 136, 4);
        js_obj.accepted = PduUtils.binToValue("bool", bin);
    }
...
/**
 * Serializes a ShareObjectOwnerResponse object into a binary PDU.
 * @param { ShareObjectOwnerResponse } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_ShareObjectOwnerResponse(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_ShareObjectOwnerResponse(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_ShareObjectOwnerResponse(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: object_name, type: string (primitive)
    ...
    {
        const bin = PduUtils.typeToBin("bool", js_obj.accepted, 4);
