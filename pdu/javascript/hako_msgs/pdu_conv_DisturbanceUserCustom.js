...
/**
 * Deserializes a binary PDU into a DisturbanceUserCustom object.
 * @param {ArrayBuffer} binary_data
 * @returns { DisturbanceUserCustom }
 */
export function pduToJs_DisturbanceUserCustom(binary_data) {
    const js_obj = new DisturbanceUserCustom();
    const meta_parser = new PduUtils.PduMetaDataParser();
    const meta = meta_parser.load_pdu_meta(binary_data);
    if (meta === null) {
        throw new Error("Invalid PDU binary data: MetaData not found or corrupted");
    }
    binary_read_recursive_DisturbanceUserCustom(meta, binary_data, js_obj, meta.base_off);
    return js_obj;
}

export function binary_read_recursive_DisturbanceUserCustom(meta, binary_data, js_obj, base_off) {
...
/**
 * Serializes a DisturbanceUserCustom object into a binary PDU.
 * @param { DisturbanceUserCustom } js_obj
 * @returns {ArrayBuffer}
 */
export function jsToPdu_DisturbanceUserCustom(js_obj) {
    const base_allocator = new PduUtils.DynamicAllocator();
    const bw_container = new PduUtils.BinaryWriterContainer(new PduUtils.PduMetaData());

    binary_write_recursive_DisturbanceUserCustom(0, bw_container, base_allocator, js_obj);
...
export function binary_write_recursive_DisturbanceUserCustom(parent_off, bw_container, allocator, js_obj) {
    const littleEndian = true;
    // member: data, type: float64 (primitive)
