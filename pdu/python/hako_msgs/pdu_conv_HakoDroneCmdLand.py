...
def pdu_to_py_HakoDroneCmdLand(binary_data: bytearray) -> HakoDroneCmdLand:
...
def binary_read_recursive_HakoDroneCmdLand(meta: binary_io.PduMetaData, binary_data: bytearray, py_obj: HakoDroneCmdLand, base_off: int):
    # array_type: single 
    # data_type: struct 
    # member_name: header 
...
def py_to_pdu_HakoDroneCmdLand(py_obj: HakoDroneCmdLand) -> bytearray:
    binary_data = bytearray()
    base_allocator = DynamicAllocator(False)
    bw_container = BinaryWriterContainer(binary_io.PduMetaData())
    binary_write_recursive_HakoDroneCmdLand(0, bw_container, base_allocator, py_obj)
...
def binary_write_recursive_HakoDroneCmdLand(parent_off: int, bw_container: BinaryWriterContainer, allocator, py_obj: HakoDroneCmdLand):
    # array_type: single 
    # data_type: struct 
    # member_name: header 
    # type_name: HakoDroneCmdHeader 
...
if __name__ == "__main__":
    ...
    def print_usage():
        print(f"Usage: python -m pdu.python.pdu_conv_HakoDroneCmdLand <read|write> [args...]")
    ...
    if command == "read":
        ...
        py_obj = pdu_to_py_HakoDroneCmdLand(binary_data)
        ...
        py_obj = HakoDroneCmdLand.from_json(json_str)
        
        binary_data = py_to_pdu_HakoDroneCmdLand(py_obj)
