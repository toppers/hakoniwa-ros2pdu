...
def pdu_to_py_Int16MultiArray(binary_data: bytearray) -> Int16MultiArray:
...
def binary_read_recursive_Int16MultiArray(meta: binary_io.PduMetaData, binary_data: bytearray, py_obj: Int16MultiArray, base_off: int):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
...
def py_to_pdu_Int16MultiArray(py_obj: Int16MultiArray) -> bytearray:
    binary_data = bytearray()
    base_allocator = DynamicAllocator(False)
    bw_container = BinaryWriterContainer(binary_io.PduMetaData())
    binary_write_recursive_Int16MultiArray(0, bw_container, base_allocator, py_obj)
...
def binary_write_recursive_Int16MultiArray(parent_off: int, bw_container: BinaryWriterContainer, allocator, py_obj: Int16MultiArray):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
    # type_name: MultiArrayLayout 
...
if __name__ == "__main__":
    ...
    def print_usage():
        print(f"Usage: python -m pdu.python.pdu_conv_Int16MultiArray <read|write> [args...]")
    ...
    if command == "read":
        ...
        py_obj = pdu_to_py_Int16MultiArray(binary_data)
        ...
        py_obj = Int16MultiArray.from_json(json_str)
        
        binary_data = py_to_pdu_Int16MultiArray(py_obj)
