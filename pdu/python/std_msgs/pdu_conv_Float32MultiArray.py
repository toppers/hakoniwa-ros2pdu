...
def pdu_to_py_Float32MultiArray(binary_data: bytearray) -> Float32MultiArray:
    py_obj = Float32MultiArray()
    meta_parser = binary_io.PduMetaDataParser()
    meta = meta_parser.load_pdu_meta(binary_data)
    if meta is None:
        raise ValueError("Invalid PDU binary data: MetaData not found or corrupted")
    binary_read_recursive_Float32MultiArray(meta, binary_data, py_obj, binary_io.PduMetaData.PDU_META_DATA_SIZE)
    return py_obj


def binary_read_recursive_Float32MultiArray(meta: binary_io.PduMetaData, binary_data: bytearray, py_obj: Float32MultiArray, base_off: int):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
...
def py_to_pdu_Float32MultiArray(py_obj: Float32MultiArray) -> bytearray:
    ...
    binary_write_recursive_Float32MultiArray(0, bw_container, base_allocator, py_obj)
...
def binary_write_recursive_Float32MultiArray(parent_off: int, bw_container: BinaryWriterContainer, allocator, py_obj: Float32MultiArray):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
    # type_name: MultiArrayLayout 
...
if __name__ == "__main__":
    ...
    def print_usage():
        print(f"Usage: python -m pdu.python.pdu_conv_Float32MultiArray <read|write> [args...]")
    ...
    if command == "read":
        ...
        py_obj = pdu_to_py_Float32MultiArray(binary_data)
        ...
        py_obj = Float32MultiArray.from_json(json_str)
        
        binary_data = py_to_pdu_Float32MultiArray(py_obj)
