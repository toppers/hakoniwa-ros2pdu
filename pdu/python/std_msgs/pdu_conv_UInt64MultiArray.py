...
def pdu_to_py_UInt64MultiArray(pdu_bytes: bytes) -> UInt64MultiArray:
    ...
    for i in range(array_len):
        ...
        val = struct.unpack_from('<Q', heap_data, current_heap_offset)[0]
...
def py_to_pdu_UInt64MultiArray(py_obj: UInt64MultiArray) -> bytes:
    ...
    for element in py_obj.data:
        elements_heap_bytes += struct.pack('<Q', element)
