...
def pdu_to_py_Int8MultiArray(binary_data: bytearray) -> Int8MultiArray:
    py_obj = Int8MultiArray()
    meta_parser = binary_io.PduMetaDataParser()
    meta = meta_parser.load_pdu_meta(binary_data)
    if meta is None:
        raise ValueError("Invalid PDU binary data: MetaData not found or corrupted")
    binary_read_recursive_Int8MultiArray(meta, binary_data, py_obj, binary_io.PduMetaData.PDU_META_DATA_SIZE)
    return py_obj


def binary_read_recursive_Int8MultiArray(meta: binary_io.PduMetaData, binary_data: bytearray, py_obj: Int8MultiArray, base_off: int):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
...
def py_to_pdu_Int8MultiArray(py_obj: Int8MultiArray) -> bytearray:
    binary_data = bytearray()
    base_allocator = DynamicAllocator(False)
    bw_container = BinaryWriterContainer(binary_io.PduMetaData())
    binary_write_recursive_Int8MultiArray(0, bw_container, base_allocator, py_obj)
...
def binary_write_recursive_Int8MultiArray(parent_off: int, bw_container: BinaryWriterContainer, allocator, py_obj: Int8MultiArray):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
    # type_name: MultiArrayLayout 
...
if __name__ == "__main__":
    ...
    def print_usage():
        print(f"Usage: python -m pdu.python.pdu_conv_Int8MultiArray <read|write> [args...]")
    ...
    if command == "read":
        ...
        py_obj = pdu_to_py_Int8MultiArray(binary_data)
        ...
        py_obj = Int8MultiArray.from_json(json_str)
        
        binary_data = py_to_pdu_Int8MultiArray(py_obj)
