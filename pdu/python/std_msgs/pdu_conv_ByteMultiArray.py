...
def pdu_to_py_ByteMultiArray(binary_data: bytearray) -> ByteMultiArray:
    py_obj = ByteMultiArray()
    meta_parser = binary_io.PduMetaDataParser()
    meta = meta_parser.load_pdu_meta(binary_data)
    if meta is None:
        raise ValueError("Invalid PDU binary data: MetaData not found or corrupted")
    binary_read_recursive_ByteMultiArray(meta, binary_data, py_obj, binary_io.PduMetaData.PDU_META_DATA_SIZE)
    return py_obj


def binary_read_recursive_ByteMultiArray(meta: binary_io.PduMetaData, binary_data: bytearray, py_obj: ByteMultiArray, base_off: int):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
...
def py_to_pdu_ByteMultiArray(py_obj: ByteMultiArray) -> bytearray:
    binary_data = bytearray()
    base_allocator = DynamicAllocator(False)
    bw_container = BinaryWriterContainer(binary_io.PduMetaData())
    binary_write_recursive_ByteMultiArray(0, bw_container, base_allocator, py_obj)
...
def binary_write_recursive_ByteMultiArray(parent_off: int, bw_container: BinaryWriterContainer, allocator, py_obj: ByteMultiArray):
    # array_type: single 
    # data_type: struct 
    # member_name: layout 
    # type_name: MultiArrayLayout 
...
if __name__ == "__main__":
    ...
    def print_usage():
        print(f"Usage: python -m pdu.python.pdu_conv_ByteMultiArray <read|write> [args...]")
    ...
    if command == "read":
        ...
        py_obj = pdu_to_py_ByteMultiArray(binary_data)
        ...
        py_obj = ByteMultiArray.from_json(json_str)
        
        binary_data = py_to_pdu_ByteMultiArray(py_obj)
