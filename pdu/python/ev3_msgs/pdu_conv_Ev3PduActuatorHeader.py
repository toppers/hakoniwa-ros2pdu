...
def pdu_to_py_Ev3PduActuatorHeader(pdu_bytes: bytes) -> Ev3PduActuatorHeader:
    ...
    # Processing: asset_time (single)
    
    
    py_obj.asset_time = struct.unpack_from('<q', base_data, 136)[0]
...
def py_to_pdu_Ev3PduActuatorHeader(py_obj: Ev3PduActuatorHeader) -> bytes:
    ...
    struct.pack_into('<q', base_buffer, 136, py_obj.asset_time)
