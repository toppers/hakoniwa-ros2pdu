...
public:
    ...
    bool pdu2cpp(char* top_ptr, HakoCpp_DisturbanceTemperature& cppData)
    {
        ...
        int ret = hako_convert_pdu2cpp_DisturbanceTemperature(*(Hako_Twist*)base_ptr, cppData);
        if (ret != 0) {
    ...
    int cpp2pdu(HakoCpp_DisturbanceTemperature& cppData, char* pdu_buffer, int buffer_len)
    {
        ...
        int pdu_size = hako_convert_cpp2pdu_DisturbanceTemperature(cppData, (Hako_Twist**)&base_ptr);
        if (pdu_size < 0) {
