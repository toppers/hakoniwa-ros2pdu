...
public:
    ...
    bool pdu2cpp(char* top_ptr, HakoCpp_UInt64& cppData)
    {
        char* base_ptr = (char*)hako_get_base_ptr_pdu((void*)top_ptr);
        if (base_ptr == nullptr) {
            std::cerr << "[ConvertorError][" << "UInt64" << "] hako_get_base_ptr_pdu returned null" << std::endl;
            return false;
        } 
        int ret = hako_convert_pdu2cpp_UInt64(*(Hako_UInt64*)base_ptr, cppData);
        if (ret != 0) {
            std::cerr << "[ConvertorError][" << "UInt64" << "] hako_convert_pdu2cpp returned " << ret << std::endl;
            return false;
        }
        return true;
    }

    int cpp2pdu(HakoCpp_UInt64& cppData, char* pdu_buffer, int buffer_len)
    {
        char* base_ptr = nullptr;
        int pdu_size = hako_convert_cpp2pdu_UInt64(cppData, (Hako_UInt64**)&base_ptr);
        if (pdu_size < 0) {
            std::cerr << "[ConvertorError][" << "UInt64" << "] hako_convert_cpp2pdu returned error code: " << pdu_size << std::endl;
            return -1;
        }
        if (pdu_size > buffer_len) {
            std::cerr << "[ConvertorError][" << "UInt64" << "] buffer too small. pdu_size=" << pdu_size << " buffer_len=" << buffer_len << std::endl;
            return -1;
        }
        void* top_ptr = hako_get_top_ptr_pdu((void*)base_ptr);
        if (top_ptr == nullptr) {
            std::cerr << "[ConvertorError][" << "UInt64" << "] hako_get_top_ptr_pdu returned null" << std::endl;
            return false;
        }
